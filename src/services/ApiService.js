...
class ApiService {
  ...
  // ─── Prop / Challenge APIs (The Funded Zone) ───
  async getChallenges() {
    return this.request('/prop/challenges');
  }
  async getPublicChallenges() {
    return this.request('/prop/public/challenges');
  }
  async getMyChallengeAccounts() {
    return this.request('/prop/my-accounts');
  }
  async buyChallenge(body) {
    // { challenge_id, fund_size, coupon_code?, purchase_type?, payment_method? }
    return this.request('/prop/buy-request', { method: 'POST', body: JSON.stringify(body) });
  }
  async validateCoupon(body) {
    // { challenge_id, fund_size, code }
    return this.request('/prop/validate-coupon', { method: 'POST', body: JSON.stringify(body) });
  }
  async completeChallengePayment(challengeAccountId) {
    return this.request('/prop/complete-payment', {
      method: 'POST', body: JSON.stringify({ challenge_account_id: challengeAccountId }),
    });
  }
  async getPropDashboard() {
    return this.request('/prop/dashboard');
  }
  async getPropLeaderboard(period = 'weekly', metric = 'profit') {
    return this.request(`/prop/leaderboard?period=${period}&metric=${metric}`);
  }
  async requestPropPayout(challengeAccountId) {
    return this.request('/prop/withdraw', {
      method: 'POST', body: JSON.stringify({ challenge_account_id: challengeAccountId }),
    });
  }
  async getMyPayouts() {
    return this.request('/prop/my-payouts');
  }
  async getCertificates() {
    return this.request('/prop/certificates');
  }
  ...
  // Social Trading - The Funded Zone format
  async getMasters() {

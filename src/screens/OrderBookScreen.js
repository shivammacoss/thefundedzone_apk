...
const OrderBookScreen = ({ navigation }) => {
  ...
  const loadData = async () => {
    try {
      ...
      const rawItems = accountsData.items || accountsData || [];
      // Hide closed/deleted accounts (backend returns them with is_active=false
      // for history) so they don't show in the picker.
      const items = (Array.isArray(rawItems) ? rawItems : []).filter(
        (a) =>
          a.is_active !== false &&
          !['closed', 'deleted', 'inactive'].includes(String(a.status || '').toLowerCase())
      );

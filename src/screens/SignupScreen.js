...
const SignupScreen = ({ navigation }) => {
  ...
  // Server-driven: only show the email-OTP step if the backend is actually
  // verifying codes (REGISTER_OTP_ENABLED). Mirrors the web register page.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const res = await fetch(`${AUTH_URL}/register/otp-status`);
        const data = await res.json().catch(() => ({}));
        if (!cancelled) setOtpEnabled(!!data?.enabled);
      } catch (_) {
  ...
  // Actual account creation. `otp` is included only when email-OTP is enabled.
  const doRegister = async (otp) => {
    setLoading(true);
    try {
      const registerData = {
  ...
  // Step 1 of email-OTP register: email a 6-digit code, then show the verify step.
  const requestOtp = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${AUTH_URL}/register/request-otp`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: formData.email.trim().toLowerCase(),
      ...
      if (!res.ok) {
        ...
        throw new Error(typeof d === 'string' ? d : 'Could not send verification code');
      }
      // Feature turned off server-side mid-flow — register without a code.
      if (data.status === 'disabled') {
        await doRegister(undefined);
        return;
      }
      setOtpMaskedEmail(data.email || formData.email);
  ...
  const handleVerifyAndRegister = async () => {
    ...
    if (!code || code.length < 4) {
      Alert.alert('Error', 'Enter the 6-digit code we emailed you');
      return;
    }
  ...
  const handleSignup = async () => {
    ...
    // When OTP is enabled, password submit kicks off the email-code step.
    if (otpEnabled) {
      await requestOtp();
      return;
    }
      ...
      >
        ...
        {step === 1 ? (
          <>
            ...
            {/* Phone (optional) */}
            <View style={styles.inputContainer}>
              <Ionicons name="call-outline" size={20} color="#8A94A6" style={styles.inputIcon} />
              <TextInput
                style={styles.input}
            ...
            <View style={styles.inputContainer}>
              <Ionicons name="gift-outline" size={20} color="#8A94A6" style={styles.inputIcon} />
              <TextInput
                style={styles.input}
          ...
          <>
            ...
            <Text style={styles.subtitle}>{otpEnabled ? 'Step 2 of 3 — Secure your account' : 'Step 2 of 2 — Secure your account'}</Text>
            ...
            <View style={styles.inputContainer}>
              ...
              <TouchableOpacity onPress={() => setShowConfirmPassword(!showConfirmPassword)} style={styles.eyeIcon}>
                <Ionicons name={showConfirmPassword ? 'eye-outline' : 'eye-off-outline'} size={20} color="#8A94A6" />
              </TouchableOpacity>
            </View>
            ...
            >
              {loading ? (
                ...
                <Text style={styles.buttonText}>{otpEnabled ? 'Continue' : 'Create Account'}</Text>
              )}
            </TouchableOpacity>
          ...
          <>
            ...
            <Text style={styles.subtitle}>
              Step 3 of 3 — Enter the 6-digit code sent to {otpMaskedEmail || formData.email}
            </Text>

            {otpDevCode ? (
              <View style={styles.summaryBox}>
                <Ionicons name="information-circle-outline" size={20} color="#1a73e8" />
                <Text style={styles.summaryText}>Dev code: {otpDevCode}</Text>
              </View>
            ) : null}

            {/* OTP Input */}
            <View style={styles.inputContainer}>
              <Ionicons name="key-outline" size={20} color="#8A94A6" style={styles.inputIcon} />
              <TextInput
                style={styles.input}
            ...
            {/* Resend */}
            <TouchableOpacity style={styles.backButton} onPress={requestOtp} disabled={loading}>
